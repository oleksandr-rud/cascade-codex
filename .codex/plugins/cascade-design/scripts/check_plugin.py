#!/usr/bin/env python3
"""Structural and contract validation for the Cascade Design package."""
...
def validate(root: Path) -> list[str]:
    ...
    if plugin.get("name") != "cascade-design" or plugin.get("name") != contract.get("plugin_id"):
    ...
    for skill in sorted(EXPECTED_SKILLS):
        ...
        for forbidden in (".codex/skills/", ".codex/agents/designer"):
    ...
    for required in (
        ...
        "specs/extraction-manifest.json", "schemas/design-review.schema.json",
    ...
    schema = load_json(root / "schemas" / "design-review.schema.json")
    if schema.get("$schema") != "https://json-schema.org/draft/2020-12/schema" or schema.get("type") != "object":
        errors.append("design-review schema declaration is invalid")
    try:
        Draft202012Validator.check_schema(schema)
    except Exception as error:
        errors.append(f"design-review schema is invalid: {error}")
    ...
    if packet.get("target_fields") != ["case_id", "request", "fixture"] or packet.get("sealed_fields") != ["skill", "expected_status", "oracle", "mechanical_assertions"]:
        errors.append("blind packet contract is invalid")
    catalog = suite.get("assertion_catalog", {})
    if set(catalog) != ASSERTIONS:
    ...
    for case in cases:
        ...
        if case.get("mechanical_assertions") != ["status_matches_expected", "selected_skill_matches_expected", "design_artifact_validates"]:
            errors.append(f"{case_id}: mechanical assertion set is incomplete")
        fixture = case.get("fixture", {})
        output = fixture.get("output_contract", {}) if isinstance(fixture, dict) else {}
        if output.get("response_encoding") != "json" or output.get("schema") != "schemas/design-review.schema.json":
