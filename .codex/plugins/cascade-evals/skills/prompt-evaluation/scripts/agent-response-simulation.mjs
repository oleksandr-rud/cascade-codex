...
export async function runAgentResponseSimulation({ phase, runId, runRoot, model, prompt, cwd, timeoutMs, adapter, adapterConfig, adapterId }) {
  ...
  const simulation = {
    ...
    brief: {
      ...
      context: [{ claim: `The phase targets ${model}`, source_refs: ["phase_prompt"] }], rules: [{ claim: "The prompt and model identity remain fixed for this phase", source_refs: ["phase_prompt"] }], non_goals: ["Judging response quality", "Changing the target prompt"]
    },
    outcome: {
  ...
  const adapterDefinition = {
    ...
    actions: [{ name: "invoke", purpose: "normal", description: "Invoke one declared model with one frozen prompt", capability: "agent.invoke", risk: "external", confirmation: "when-not-authorized", idempotency: "key-required", input: { type: "object", additionalProperties: false, required: ["model", "prompt_sha256"], properties: { model: { type: "string", minLength: 1 }, prompt_sha256: { type: "string", pattern: "^[a-f0-9]{64}$" } } } }],
  ...
  const authorization = controllerCall(["authorize", "--run-dir", controllerRoot, "--action", "invoke", "--input-json", JSON.stringify({ model, prompt_sha256: promptDigest }), "--idempotency-key", `${runId}:${phase}:${promptDigest}`, "--expected-tool-calls", "1"]);
  const run = runModel({ model, prompt, cwd, timeoutMs, adapter, adapterConfig, adapterId });
  const completed = run.status === "COMPLETED";
  const outputDigest = completed ? sha256(run.final_text) : null;
  controllerCall(["record", "--run-dir", controllerRoot, "--dispatch-token", authorization.dispatch_token, "--outcome", completed ? "PASS" : run.status === "TIMED_OUT" ? "BLOCKED" : "FAIL", "--observation-name", "agent_response", "--observation", completed ? `model:${model} output_sha256:${outputDigest}` : `model:${model} status:${run.status}`, "--evidence-json", JSON.stringify([{ condition: "The declared model returned a complete response", status: completed ? "SUPPORTED" : "UNSUPPORTED", evidence: completed ? `output_sha256:${outputDigest}` : `execution_status:${run.status}` }]), "--duration-seconds", String(run.duration_ms / 1000)]);
