...
for(const item of catalog.cases.filter(c=>selected.includes(c.id))){
 ...
 const run=await runModelPhase({phase:item.id,runId,runRoot:root,model:contract.model,reasoningEffort:"max",prompt:request,cwd:workspace,timeoutMs:positiveTimeout(value("judge-timeout-ms"),DEFAULT_TIMEOUTS_MS.judge,"--judge-timeout-ms"),adapter:"codex-cli"});
