...
for (let index = 0; index < repetitions; index += 1) {
  ...
  const result = await runCommand({ command: process.execPath, args: [runner, "run", ...forwarded, "--no-prompt-cache", "--no-trajectory-cache", "--output-dir", outputRoot, "--run-id", runId], input: "", timeoutMs, signal, terminationGraceMs: 2000 });
