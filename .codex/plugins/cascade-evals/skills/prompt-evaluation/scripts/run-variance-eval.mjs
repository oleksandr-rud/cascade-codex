...
for (let index = 0; index < repetitions; index += 1) {
  ...
  const result = spawnSync(process.execPath, [runner, "run", ...forwarded, "--output-dir", outputRoot, "--run-id", runId], { encoding: "utf8", env: process.env });
  let receipt = {};
...
const completed = records.filter((record) => record.summary?.acceptance && record.summary.acceptance !== "NOT_RUN");
...
const aggregate = {
  ...
  status: records.some((record) => record.summary?.status === "BLOCKED" || !record.summary) ? "PARTIAL" : "COMPLETE",
