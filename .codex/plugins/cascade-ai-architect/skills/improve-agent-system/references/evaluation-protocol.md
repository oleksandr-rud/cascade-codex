...
Require one complete, digest-bound Cascade Evals receipt for each side of every
pair. Each receipt must bind the frozen target run and subject digest, preserve
mechanical `PASS`, include exactly the `architecture-outcome` and
`architecture-trajectory` profiles, and use distinct judge identities and
contexts. Cascade Evals validates raw rating arrays, blindness, response shape,
dimension coverage, floors, and verdict agreement, then recomputes the
conservative score. The improvement reducer consumes only that terminal
receipt; it does not accept or reduce raw judge output.
...
candidates. They may not exceed the canonical `prepare-agent-evaluation` suite
