...
- Every retained agent owns at least one capability, is referenced by at least one role, and declares non-empty inputs, outputs, workflows, prompts, and evaluations. `tools` and `skills` are required lists but may be empty; never fabricate tool access or a separate skill for a focused prompt-only agent. Capabilities listed by an agent name that agent as their primary owner.
- `single_agent` and `single_agent_with_skills` contain exactly one agent; model-pipeline, manager, decentralized-handoff, and evaluator-optimizer topologies contain multiple role-backed agents; deterministic workflow topology contains none.
- `single_agent_with_skills` additionally requires its agent to reference at least one real skill; use `single_agent` when no separate skill is justified.
- Use `model_pipeline` for runtime-routed model stages with distinct contracts, including Analyzer and Main Composer. Deterministic policy/state/context functions belong to `components.workflows`, not `components.agents`. Add Voice Composer only when model-backed, and Researcher only when selected; optional runtime activation is recorded in the workflow. The topology label and schema checks do not prove the pattern's semantic or runtime invariants.
