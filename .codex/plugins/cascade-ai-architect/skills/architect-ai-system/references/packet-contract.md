...
- Every retained agent owns at least one capability, is referenced by at least one role, and declares non-empty inputs, outputs, skills, workflows, prompts, and evaluations. `tools` is required but may be empty for proposal or composition roles; tool access must never be fabricated. Capabilities listed by an agent name that agent as their primary owner.
