#!/usr/bin/env python3
"""Generate or verify the Cascade QA evaluation asset manifest."""
...
def aggregate_digest(assets: list[dict[str, str]]) -> str:
    payload = "".join(f"{item['path']}\0{item['sha256']}\n" for item in sorted(assets, key=lambda item: item["path"])).encode()
...
def installed_plugins() -> dict[str, dict[str, Any]]:
    process = subprocess.run(["codex", "plugin", "list", "--json"], check=True, capture_output=True, text=True)
    return {item["name"]: item for item in json.loads(process.stdout).get("installed", []) if item.get("installed") and item.get("enabled")}
...
def dependency_assets(contract: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for requirement in contract["required_dependency_artifacts"]:
        ...
        root = Path.home() / ".codex" / "plugins" / "cache" / plugin["marketplaceName"] / plugin_name / plugin["version"]
        path = root / requirement["path"]
        if not path.is_file():
            raise ValueError(f"dependency artifact is unavailable: {alias} {requirement['path']}")
        result.append({"alias": alias, "plugin_version": plugin["version"], "path": requirement["path"], "sha256": sha256(path)})
    return result
...
def build(root: Path) -> dict[str, Any]:
    ...
    return {"schema_version": 2, "manifest_id": f"{contract['plugin_id']}-evaluation-assets-v2", "plugin_id": contract["plugin_id"], "plugin_version": contract["version"], "digest_algorithm": "sha256", "subject_digest_algorithm": "sha256-path-digest-list-v1", "subject_digest": aggregate_digest(subject), "subject_assets": subject, "evaluation_assets": evaluation, "dependencies": dependency_assets(contract)}
...
def verify(root: Path, manifest: dict[str, Any]) -> list[str]:
    try:
        ...
        return [str(error)]
    fields = ("schema_version", "manifest_id", "plugin_id", "plugin_version", "digest_algorithm", "subject_digest_algorithm", "subject_digest", "subject_assets", "evaluation_assets", "dependencies")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.check:
            errors = verify(args.root, json.loads(output.read_text(encoding="utf-8")))
