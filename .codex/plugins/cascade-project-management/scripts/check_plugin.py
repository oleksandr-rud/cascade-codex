#!/usr/bin/env python3
"""Validate the Cascade Project Management package."""
...
EXPECTED_SKILLS = {"define-work-item", "plan-project", "manage-project", "close-project"}
EXPECTED_ASSERTIONS = {"status_matches_expected", "selected_skill_matches_expected", "project_artifact_validates"}
...
def validate(root: Path = ROOT) -> list[str]:
    ...
    if plugin.get("name") != "cascade-project-management" or contract.get("plugin_id") != plugin.get("name"):
    ...
    schema = load(root / "schemas" / "project-management-artifact.schema.json")
    try:
    ...
    for case in cases:
        ...
        if case.get("mechanical_assertions") != ["status_matches_expected", "selected_skill_matches_expected", "project_artifact_validates"]:
            errors.append(f"{case_id}: assertion set is incomplete")
        output = case.get("fixture", {}).get("output_contract", {})
        if output.get("response_encoding") != "json" or output.get("schema") != "schemas/project-management-artifact.schema.json" or output.get("artifact_status_contract") != "Domain status is the serialized response artifact.status; outer case status is transport-only.":
    ...
    if len(list((root / "evals").glob("judge-*.json"))) != 2:
