...
def validate_v2(root: Path) -> list[str]:
    ...
    for case in cases:
        ...
        expected_fixture_fields = {"supplied", "absent", "permissions", "data"}
        if requires_typed_artifact:
            expected_fixture_fields.add("output_contract")
        if case_id == "MKT-004":
        ...
        if not isinstance(fixture, dict) or set(fixture) != expected_fixture_fields:
            ...
            if case_id == "MKT-004":
                ...
                if (
                    ...
                    or set(frozen[0]) != {"kind", "sha256", "artifact"}
                    or frozen[0].get("kind") != "EVIDENCE_LEDGER"
                    or re.fullmatch(r"[a-f0-9]{64}", str(frozen[0].get("sha256", ""))) is None
                    or not isinstance(frozen[0].get("artifact"), dict)
                ):
