#!/usr/bin/env python3
"""Generate or verify the Cascade Security evaluation asset manifest."""
...
def dependency_assets(contract: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for requirement in contract["required_dependency_artifacts"]:
        ...
        if not cache_root.is_dir():
            raise ValueError(f"immutable installed dependency cache is unavailable: {plugin_name} {plugin['version']}")
...
def verify(root: Path, manifest: dict[str, Any]) -> list[str]:
    try:
        ...
        return [str(error)]
    fields = (
        "schema_version", "manifest_id", "plugin_id", "plugin_version",
        "digest_algorithm", "subject_digest_algorithm", "subject_digest",
        "subject_assets", "evaluation_assets", "dependencies",
    )
