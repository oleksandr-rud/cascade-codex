---
name: codebase-audit
description: Audit a target codebase through evidence-bound security trajectories, stack and surface inventory, and review-only findings without exposing sensitive values or patching runtime code.
---
...
- the request is only about JWT, sessions, RBAC, or tenant isolation; use
  `cascade-security:auth-analysis` first and this skill only as supporting
  inventory;
- the request is a design review for a proposed feature; use
  `cascade-security:secure-design`;
- the user asks to implement fixes; return a bounded `target-host` handoff after
  the audit.
...
3. Current code and config discovered from target manifests, source maps,
   routes, middleware, service layers, API clients, auth modules,
   environment/config loaders, audit/logging utilities, workers, and
   integration adapters.
4. Active security specs when the request names or implies one.
5. Source-backed security references supplied by the user or stored in the
   target repository.
...
`codebase-audit` and `coverage.kind` set to `codebase`.
...
validation:
  ...
  evidence: []
next:
  action: continue | target-host | cascade-security:auth-analysis | cascade-security:secure-design | stop
  reason: string
```
