#!/usr/bin/env python3
"""Validate Cascade Market artifacts and their bound dependencies."""
...
def validate_ledger(value: dict[str, Any]) -> list[str]:
    ...
    for claim in claims:
        ...
        if value.get("status") == "READY" and source_type not in {"HYPOTHESIS", "INFERENCE"}:
            if claim.get("source_sha256") is None:
                errors.append(f"READY evidence claim lacks source digest: {claim_id}")
            if claim.get("confidence") is None:
                errors.append(f"READY evidence claim lacks confidence: {claim_id}")
    ...
    for contradiction in contradictions:
        ...
        sources = {
            (item.get("source_id"), item.get("source_locator"), item.get("source_sha256"))
            for item in referenced_claims
        }
...
def validate_handoff(value: dict[str, Any], *, now: datetime | None = None) -> list[str]:
    ...
    elif not producer.startswith("cascade-market:"):
        errors.append("Market handoff producer must be owned by Cascade Market")
    ...
    try:
        ...
        if consumer_type == "CASCADE_PLUGIN":
            ...
            frozen_missing_consumer = (
                status == "BLOCKED"
                and failure.get("classification") == "MISSING_INPUT"
                and isinstance(consumer, str)
                and isinstance(value.get("consumer_version"), str)
                and re.fullmatch(r"\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?", value["consumer_version"]) is not None
                and isinstance(resume.get("required_artifact"), str)
                and consumer in resume["required_artifact"]
            )
            if expected_consumer_version is None:
                if not frozen_missing_consumer:
                    errors.append(f"handoff consumer alias is not bound by the frozen package manifest: {consumer}")
            elif value.get("consumer_version") != expected_consumer_version and not frozen_missing_consumer:
                errors.append(
    ...
    if mode == "EXECUTE" and (
        ...
        or not producer.startswith("cascade-market:")
    ...
    if mode == "EXECUTE" and isinstance(consumer, str) and consumer.startswith("cascade-market:"):
        errors.append("Cascade Market cannot be the execution consumer")
    try:
