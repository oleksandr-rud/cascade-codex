...
def executed_from_frozen(wrapper: dict) -> dict:
    ...
    value["input_artifacts"] = [{
        ...
        "alias": "cascade-market:design-market-experiments",
        "status": "READY",
...
class MarketAssertionAdapterTests(unittest.TestCase):
        ...
        for case in self.suite["cases"]:
            ...
            if "market_artifact_valid" in case["mechanical_assertions"]:
                ...
                if case["skill"] == "research-market":
                    ...
                    frozen_inputs = case["fixture"].get("frozen_inputs", [])
                    if frozen_inputs:
                        supporting_artifacts = copy.deepcopy(frozen_inputs)
                        instrument_wrapper = next(
                            item for item in supporting_artifacts
                            if item["kind"] == "EXPERIMENT_INSTRUMENT"
                        )
                        instrument_source = instrument_wrapper["artifact"]
                        artifact["instrument"] = {
                            "id": instrument_source["id"],
                            "version": instrument_source["version"],
                            "sha256": instrument_wrapper["sha256"],
                            "kind": instrument_source["kind"],
                        }
                        artifact["instrumentation"] = [
                            {
                                "event_id": item["artifact"]["event_id"],
                                "trigger": f"Record {item['artifact']['event_id']} under the frozen event schema.",
                                "schema_sha256": item["sha256"],
                            }
                            for item in supporting_artifacts
                            if item["kind"].startswith("EVENT_SCHEMA_")
                        ]
                handoffs = []
            ...
            elif "brand_output_valid" in case["mechanical_assertions"]:
                source_bindings = [
                    {
                        **copy.deepcopy(source),
                        "supports": ["Only the support explicitly supplied by the frozen fixture."],
                        "limitations": "The source cannot authorize product behavior, implementation, publication, or external effects.",
                    }
                    for source in case["fixture"].get("frozen_sources", [])
                ]
                response = json.dumps({
                    "disposition": case["expected_status"],
                    "content": {"candidate_state": case["expected_status"]},
                    "source_bindings": source_bindings,
                    "authority_boundaries": {
                        "cascade-market:brand-positioning": "Owns reviewable marketing language only.",
                    },
                    "handoffs": [
                        {
                            "owner": owner,
                            "required_artifact": "The exact accepted artifact required by the named owner.",
                            "next_action": "The named owner decides or executes this downstream phase.",
                            "status": "NOT_RUN",
                        }
                        for owner in case["fixture"].get("required_handoff_owners", [])
                    ],
                }, separators=(",", ":"), sort_keys=True)
            results.append({
    ...
    def test_experiment_instrument_and_event_schemas_require_exact_frozen_bytes(self) -> None:
        target = self.target()
        case = next(item for item in target["cases"] if item["case_id"] == "MKT-006")
        envelope = json.loads(case["response"])
        envelope["artifact"]["instrument"]["sha256"] = "f" * 64
        envelope["artifact_sha256"] = canonical_digest(envelope["artifact"])
        case["response"] = json.dumps(envelope)
        self.assertEqual(evaluate(self.suite, target)["status"], "INVALID")

        target = self.target()
        case = next(item for item in target["cases"] if item["case_id"] == "MKT-014")
        envelope = json.loads(case["response"])
        envelope["artifact"]["instrumentation"][0]["schema_sha256"] = "f" * 64
        envelope["artifact_sha256"] = canonical_digest(envelope["artifact"])
        case["response"] = json.dumps(envelope)
        self.assertEqual(evaluate(self.suite, target)["status"], "INVALID")

    def test_blocked_handoff_binds_the_finalized_experiment_digest(self) -> None:
        target = self.target()
        case = next(item for item in target["cases"] if item["case_id"] == "MKT-013")
        envelope = json.loads(case["response"])
        handoff = envelope["handoffs"][0]["artifact"]
        handoff["input_artifacts"][0]["sha256"] = "0" * 64
    ...
    def test_brand_output_contract_and_frozen_source_identity_fail_closed(self) -> None:
        target = self.target()
        brand_case = next(item for item in target["cases"] if item["case_id"] == "MKT-015")
        output = json.loads(brand_case["response"])
        output["disposition"] = {"status": "PROPOSED"}
        brand_case["response"] = json.dumps(output)
        self.assertEqual(evaluate(self.suite, target)["status"], "INVALID")

        target = self.target()
        brand_case = next(item for item in target["cases"] if item["case_id"] == "MKT-015")
        output = json.loads(brand_case["response"])
        output["source_bindings"][0]["sha256"] = "0" * 64
        brand_case["response"] = json.dumps(output)
        self.assertEqual(evaluate(self.suite, target)["status"], "INVALID")
        ...
        brand_case = next(item for item in target["cases"] if item["case_id"] == "MKT-015")
        output = json.loads(brand_case["response"])
        output["source_bindings"][0]["id"] = ["unhashable", "identity"]
        output["handoffs"][0]["owner"] = ["unhashable", "owner"]
        brand_case["response"] = json.dumps(output)
        self.assertEqual(evaluate(self.suite, target)["status"], "INVALID")
