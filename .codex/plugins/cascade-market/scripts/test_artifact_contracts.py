...
def experiment() -> dict:
    return {
        ...
        "input_artifacts": [{
            ...
            "version": "1",
            "sha256": DIGEST,
            "alias": "cascade-market:research-market",
            "status": "READY",
...
def handoff() -> dict:
    ...
    return {
        ...
        "producer": "cascade-market:evaluate-market-opportunity",
        "producer_version": versions["cascade-market:evaluate-market-opportunity"],
        ...
        "resume": {
            ...
            "next_action": "Review the prepared Market artifact and record the bounded Product decision.",
        },
...
class MarketArtifactContractTests(unittest.TestCase):
    ...
    def test_absent_baseline_and_unavailable_source_metadata_remain_explicit(self) -> None:
        root = SCRIPT_ROOT.parent
        no_baseline = experiment()
        no_baseline["baseline"] = None
        self.assertEqual(validate_schema(no_baseline, root / "schemas" / "experiment-contract.schema.json"), [])
        self.assertEqual(validate_experiment(no_baseline), [])

        incomplete = ledger()
        incomplete["status"] = "GAP"
        incomplete["gaps"] = ["Exact source bytes, digest, retrieval time, and confidence are unavailable."]
        incomplete["claims"] = [incomplete["claims"][0]]
        incomplete["claims"][0]["source_sha256"] = None
        incomplete["claims"][0]["confidence"] = None
        incomplete["claims"][0]["freshness_status"] = "UNKNOWN"
        incomplete["claims"][0]["retrieved_at"] = None
        self.assertEqual(validate_schema(incomplete, root / "schemas" / "evidence-ledger.schema.json"), [])
        self.assertEqual(validate_ledger(incomplete), [])

        current_without_retrieval = copy.deepcopy(incomplete)
        current_without_retrieval["claims"][0]["freshness_status"] = "CURRENT"
        self.assertTrue(any(
            "CURRENT claim lacks retrieved_at" in error
            for error in validate_ledger(current_without_retrieval)
        ))

        incomplete["status"] = "READY"
        incomplete["gaps"] = []
        errors = validate_ledger(incomplete)
        self.assertTrue(any("lacks source digest" in error for error in errors))
        self.assertTrue(any("lacks confidence" in error for error in errors))
    ...
    def test_gap_contradiction_accepts_distinct_labeled_sources_without_locators(self) -> None:
        value = ledger()
        value["status"] = "GAP"
        value["gaps"] = ["External source locators, bytes, timestamps, and a governing rule are unavailable."]
        vendor = claim("vendor-price", "vendor-pricing-page", "VENDOR_CLAIM")
        vendor["statement"] = "The described vendor page quotes one unspecified price for the supplied plan and geography."
        vendor["source_type"] = "VENDOR_CLAIM"
        invoice = claim("invoice-price", "signed-customer-invoice", "PUBLIC_FACT")
        invoice["statement"] = "The described signed invoice records a different unspecified transaction price for the same plan and geography."
        for item in (vendor, invoice):
            item["source_locator"] = None
            item["source_sha256"] = None
            item["published_at"] = None
            item["retrieved_at"] = None
            item["confidence"] = None
            item["freshness_status"] = "UNKNOWN"
            item["contradiction_ids"] = ["price-conflict"]
        value["claims"] = [vendor, invoice]
        value["contradictions"] = [{
            "contradiction_id": "price-conflict",
            "claim_ids": ["vendor-price", "invoice-price"],
            "resolution_status": "UNRESOLVED",
            "governing_rule": None,
        }]
        self.assertEqual(validate_schema(value, SCRIPT_ROOT.parent / "schemas" / "evidence-ledger.schema.json"), [])
        self.assertEqual(validate_ledger(value), [])

    def test_gap_contradiction_rejects_one_labeled_source_without_locators(self) -> None:
        value = ledger()
        value["status"] = "GAP"
        value["gaps"] = ["External source identity and a governing rule are unavailable."]
        first = claim("price-a", "unknown-price-source", "PUBLIC_FACT")
        second = claim("price-b", "unknown-price-source", "PUBLIC_FACT")
        first["statement"] = "The source states price A."
        second["statement"] = "The source states price B."
        for item in (first, second):
            ...
            item["freshness_status"] = "UNKNOWN"
            item["contradiction_ids"] = ["price-conflict"]
        value["claims"] = [first, second]
        value["contradictions"] = [{
            "contradiction_id": "price-conflict",
            "claim_ids": ["price-a", "price-b"],
            "resolution_status": "UNRESOLVED",
            "governing_rule": None,
        }]
        self.assertTrue(any("substantively distinct" in error for error in validate_ledger(value)))
    ...
    def test_blocked_missing_dependency_preserves_frozen_consumer_version(self) -> None:
        value = handoff()
        value["consumer"] = "cascade-prompt:prompt"
        value["consumer_version"] = "0.6.0+codex.20260820015508"
        value["status"] = "BLOCKED"
        value["output_artifacts"] = []
        value["failure"] = {
            "classification": "MISSING_INPUT",
            "message": "The required dependency is disabled.",
            "retry_allowed": True,
        }
        value["resume"] = {
            "owner": "dependency-owner:cascade-prompt",
            "required_artifact": "enabled cascade-prompt:prompt registry receipt",
            "not_applicable_reason": None,
            "next_action": "Enable the exact dependency and rerun the frozen design.",
        }
        errors = validate_handoff(value)
        self.assertFalse(any("consumer_version" in error for error in errors))
