...
An external-action request does not change this classification: when the
visible inputs are sufficient to freeze the experiment, return case status
READY, refuse the requested launch, and keep the artifact `execution_state` at
NOT_RUN. The absence of a separate operator authority or execution receipt is
an execution boundary, not a design GAP or BLOCKED condition. Never return GAP solely because the request says to launch, spend, publish, contact prospects, or execute now.
...
The v4 `baseline` key is nullable: use `null` whenever a comparison baseline is
not applicable or no baseline value and source artifact were supplied. A kill
threshold, observed result, or experiment identity is not a baseline and must
never be copied into that field.
...
5. Record required authority for external outreach, publication, spend, data collection, or account mutation, but do not execute it from this plugin. A Market-produced `EXECUTE` handoff may delegate only to a named `external-operator:<id>` consumer with a bound producer/consumer version, expected output identity/status, destination, and bounded model/tool/write/time budget; Cascade Market is never the execution consumer.
6. Emit a v4 execution packet conforming to ../../schemas/experiment-contract.schema.json with frozen input lineage, recruitment, privacy, instrument, baseline, duration, instrumentation, contamination controls, and execution_state NOT_RUN. Every instrument and event-schema digest must resolve to byte-exact JSON in `supporting_artifacts`; bind the instrument ID/version/kind and each event ID to the matching wrapper digest. Never use an all-zero, repeated-character, opaque, or remembered digest when the corresponding bytes are unavailable: return GAP or BLOCKED and name the owner required to freeze them. Validate the packet with `uv run --offline --with jsonschema python ../../scripts/validate_artifact.py experiment EXPERIMENT.json --schema ../../schemas/experiment-contract.schema.json`; overlapping success/kill thresholds, duplicate metric identities, missing operational lineage, unverifiable instrument bytes, or a forged execution receipt are INVALID. A separate operator may execute only under its own authority and must return an identity-, permission-, cost-, cleanup-, and evidence-bound receipt.
...
If those terms omit a baseline, preserve that absence as `baseline: null`.
...
Design readiness is not execution authority. Return READY when the design,
instrument bytes, event schemas, lineage, thresholds, sample, stopping rule,
permissions, and operator boundary are complete enough to freeze, even when a
separate execution authority or operator receipt is absent. Preserve that
absence as `execution_state: NOT_RUN`, false execution/spend permissions, and
an exact execution-owner requirement. Return GAP only when an input required
to freeze the design itself is missing, and BLOCKED only when a required
dependency or owner action prevents completing the design. READY never grants
outreach, publication, spend, collection, or external-write authority.
...
For a failed missing-dependency handoff, the unavailable consumer is evidence,
not an enabled call target. If the visible fixture supplies its frozen disabled
alias and exact semantic version, preserve that snapshot in `consumer` and
`consumer_version`, close recovery on a future enabled-registry receipt, and do
not replace it with the currently installed version. Successful handoffs still
use the current manifest-bound identity. A successful READY PREPARE handoff
must name its deferred resume owner, next action, and exactly one required
artifact or not-applicable reason; only terminal acknowledged PASS uses null
resume fields.
...
A BLOCKED PREPARE handoff likewise binds the emitted experiment as exactly one
DERIVED/BLOCKED input artifact by its matching ID, version, and finalized outer
digest. It must not use a remembered, opaque, or unrelated digest.
...
digests. When the visible fixture contains `frozen_inputs`, copy every wrapper
exactly into `supporting_artifacts` and bind the instrument and event-schema
references to those wrapper digests. Otherwise use `supporting_artifacts: []`;
the experiment's typed `input_artifacts`, embedded receipt, and handoff carry
the remaining operative lineage.
...
tool-free target, use lowercase 64-character placeholders only for digest
leaves that the finalizer explicitly recomputes. Never use placeholders for an
instrument, event schema, frozen input, or other source identity. The
manifest-bound `finalize_target` hook may
