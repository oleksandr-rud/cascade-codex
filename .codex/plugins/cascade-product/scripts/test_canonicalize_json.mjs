...
assert.equal(canonicalDigest(value).sha256, crypto.createHash("sha256").update(canonical, "utf8").digest("hex"));
assert.throws(() => canonicalize("\ud800"), /lone high surrogate/);
assert.equal(canonicalize({ b: 1, a: 2 }), '{"a":2,"b":1}');
const fixture = path.join(os.tmpdir(), `cascade-product-duplicate-${process.pid}.json`);
