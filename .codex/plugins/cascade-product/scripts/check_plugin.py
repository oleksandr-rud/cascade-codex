...
def validate(root: Path) -> list[str]:
    ...
    for alias in contract.get("routed_peer_aliases", []):
        if alias not in joined:
            errors.append(f"routed peer alias is not bound: {alias}")
...
def validate_v2(root: Path) -> list[str]:
    ...
    for alias in contract.get("routed_peer_aliases", []):
        if alias not in joined:
            errors.append(f"routed peer alias is not operative in a runtime skill: {alias}")
    for phrase in contract.get("required_runtime_phrases", []):
        if phrase not in joined:
            errors.append(f"required runtime phrase is not operative: {phrase}")
    for phrase in contract.get("forbidden_runtime_phrases", []):
    ...
    if set(catalog) != required_assertions:
        errors.append("Product mechanical assertion catalog is incomplete")
        catalog = {}
    ...
    allowed_statuses = {"READY", "GAP", "BLOCKED", "INVALID", "PROPOSED", "PENDING_APPROVAL", "APPROVED"}
    for case in cases:
        case_id = case.get("case_id")
        ...
        if not all(isinstance(case.get(field), str) and case[field] for field in ("request", "oracle")):
            errors.append(f"{case_id}: request and oracle are required")
        if case.get("mechanical_assertions") != [
            "status_matches_expected",
            "selected_skill_matches_expected",
            "product_artifacts_validate",
        ]:
            errors.append(f"{case_id}: Product artifact mechanical gates are incomplete")
        ...
        optional_fixture_fields = {"input_bindings", "decision_owner"}
        if (
            not isinstance(fixture, dict)
            or not required_fixture_fields.issubset(fixture)
            or not set(fixture).issubset(required_fixture_fields | optional_fixture_fields)
        ):
            errors.append(f"{case_id}: fixture contract is incomplete")
        else:
            input_bindings = fixture.get("input_bindings")
            if input_bindings is not None and (
                not isinstance(input_bindings, list)
                or not input_bindings
                or any(
                    not isinstance(binding, dict)
                    or set(binding)
                    != {"artifact_id", "version", "sha256", "status", "freshness", "accepted_by"}
                    or not all(
                        isinstance(binding.get(field), str) and binding[field]
                        for field in ("artifact_id", "version", "status", "freshness", "accepted_by")
                    )
                    or re.fullmatch(r"[a-f0-9]{64}", str(binding.get("sha256", ""))) is None
                    for binding in input_bindings
                )
            ):
                errors.append(f"{case_id}: input bindings are invalid")
            decision_owner = fixture.get("decision_owner")
            if decision_owner is not None and (not isinstance(decision_owner, str) or not decision_owner):
                errors.append(f"{case_id}: decision_owner is invalid")
            ...
            if (
                ...
                or output_contract.get("required_response_fields")
                != ["product_artifact", "product_artifact_sha256", "decision", "decision_sha256", "handoffs"]
                or output_contract.get("handoff_wrapper_fields") != ["artifact", "sha256"]
                or output_contract.get("product_artifact_schema") != "schemas/product-work-product.schema.json"
                or output_contract.get("expected_artifact_kind") != {
                    "define-product": "PRODUCT_DEFINITION",
                    "manage-product-lifecycle": "LIFECYCLE_RECORD",
                    "validate-product": "VALIDATION_REPORT",
                }.get(skill)
                or output_contract.get("decision_schema") != "schemas/product-decision.schema.json"
                or output_contract.get("handoff_schema") != "schemas/handoff-envelope.schema.json"
                or not isinstance(output_contract.get("require_handoff"), bool)
            ):
                errors.append(f"{case_id}: output artifact contract is invalid")
    missing_categories = set(contract.get("required_case_categories", [])) - categories
    if missing_categories:
        errors.append(f"missing case categories: {sorted(missing_categories)}")
