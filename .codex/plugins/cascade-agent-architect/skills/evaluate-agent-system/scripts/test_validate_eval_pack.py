...
class EvalPackValidationTests(unittest.TestCase):
    ...
    def test_judge_array_evidence_contract_is_required(self) -> None:
        def remove_evidence(value):
            value["properties"]["ratings"]["items"]["properties"]["evidence"]["minItems"] = 0

        self.mutate("references/judge-response.schema.json", remove_evidence)
        errors = VALIDATOR.validate(self.root)
        self.assertTrue(any("evidence locators" in error for error in errors))
