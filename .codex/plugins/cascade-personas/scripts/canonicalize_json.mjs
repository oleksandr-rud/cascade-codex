  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (code >= 0xd800 && code <= 0xdbff) {
      ...
      if (!(next >= 0xdc00 && next <= 0xdfff)) {
        throw new Error("JCS rejects a lone high surrogate");
      }
      index += 1;
...
function main(argv) {
  ...
  const duplicateCheck = spawnSync(
    "python3",
    [
      "-c",
      [
  ...
  if (duplicateCheck.status !== 0) {
    throw new Error(`I-JSON validation failed: ${duplicateCheck.stderr.trim() || "duplicate member"}`);
  }
