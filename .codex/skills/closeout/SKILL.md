...
7. Treat lane completion, Coordination Graph completion, and the user's overall
   ...
   gate can accept.
   When all required current-source criteria, dependencies, and validation
   gates pass, synchronize the in-scope lane, graph, receipt, next gate, and
   active-registry projection without requesting another confirmation.
...
18. For every lane or graph that becomes complete in this closeout, first
   ...
   resolved, and remove its completed or superseded derived active row instead
   of re-marking it as `CLOSED`. Incomplete or blocked work retains its current
   state and is not archive-eligible.
