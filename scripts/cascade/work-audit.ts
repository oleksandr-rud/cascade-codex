...
Stay on the root execution surface. Do not create or dispatch agents, tasks, threads, worktrees, branches, commits, pushes, pull requests, releases, or external tracker changes. Do not run paid/provider-backed, live, destructive, secret-bearing, or other externally mutating actions. Do not run broad harness evaluation unless actual harness assertions or evaluation contracts changed and the accepted evidence scope requires it. Stop and report the exact blocker when work needs new authority, an independent evaluator, ambiguous ownership, an external action, or a dependency that is not accepted.
