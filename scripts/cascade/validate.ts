...
const REQUIRED_FILES = [
  ...
  "evals/campaigns/harness-static-smoke.json",
  "evals/campaigns/browser-simulation-smoke.json",
  "evals/tasks/schema.json",
  "evals/tasks/browser-task-smoke.json",
...
async function validateCampaigns(errors: string[]): Promise<void> {
  ...
  for (const path of await walkFiles(rootPath("evals/campaigns"), {
    include: (item) => item.endsWith(".json") && !item.endsWith("schema.json"),
  })) {
    const campaign = await readJson<Record<string, any>>(path);
    if (campaign.schema_version !== 1) errors.push(`${rel(path)} schema_version must be 1`);
    if (!campaign.id) errors.push(`${rel(path)} missing id`);
    else if (ids.has(campaign.id)) errors.push(`duplicate campaign id: ${campaign.id}`);
    else ids.add(campaign.id);
    const taskIds = new Set<string>();
    for (const entry of campaign.tasks ?? []) {
      if (
        entry.task_file &&
        (typeof entry.task_file !== "string" ||
          !entry.task_file.startsWith("evals/tasks/") ||
          !(await isFile(rootPath(entry.task_file)))
        )
      ) {
        errors.push(`${rel(path)} invalid task_file: ${entry.task_file}`);
        continue;
      }
      const task = entry.task_file
        ? await readJson<Record<string, any>>(rootPath(entry.task_file))
        : entry;
      if (taskIds.has(task.id)) errors.push(`${rel(path)} duplicate task: ${task.id}`);
      taskIds.add(task.id);
      if (!["command", "browser", "agent-response"].includes(task.kind)) {
        errors.push(`${rel(path)} invalid task kind: ${task.kind}`);
      }
      if (!Array.isArray(task.command) || !task.command.length) {
        errors.push(`${rel(path)} task ${task.id} missing command`);
      }
    }
  }
}
...
async function validateBunCutover(errors: string[]): Promise<void> {
  const roots = [
    rootPath("scripts"),
    rootPath(".codex/agents"),
    rootPath("evals/harness/fixtures/onboarding"),
  ];
  for (const root of roots) {
    for (const path of await walkFiles(root, {
      include: (item) => item.endsWith(".py"),
    })) {
      errors.push(`active Python harness tooling remains after Bun cutover: ${rel(path)}`);
    }
  }
}
