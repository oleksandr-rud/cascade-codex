...
| `W-031` | `IN_REVIEW` | Implement universal task admission, claim extraction, multi-axis workload profiling, policy compilation, proportional controls, and long-running reclassification | `agent-engineer` | independent GF-004/GF-008/GF-009/GF-101 and harness review, then G6 join | `.codex/task-admission/`; admission compiler/CLI/tests; harness admission evals; advisory and hard-control hooks; route/config/default consumers | W-032 consumes its Task Envelope; task-admission and campaign action-policy authorities remain separate | plan revision 4/lane graph revision 2; policy bundle `cascade-core@2`, 11 policies, 10 controls, 14/14 shadow trajectories with zero over/under-control, 19 focused tests, offline hook fixtures, validators and local regression pass; product and persona simulation requests select connected delivery plus simulation governance; live harness coverage is `0/368` after source invalidation and independent acceptance remains `NOT_RUN` |
