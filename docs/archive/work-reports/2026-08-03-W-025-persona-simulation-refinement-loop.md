...
|---|---|---|---|
| `PER-001` | Given an approved manifest with matching persona and derivation digests, when population derivation is previewed, then deterministic population v2 JSON is returned without writing files. | CLI/unit test | pass |
| `PER-002` | Given a stale persona digest, unapproved review, model-backed generator, or ambiguous match, when derivation is requested, then it fails closed. | negative tests | pass |
| `PER-003` | Given a legacy population v1 campaign, when definitions resolve, then current behavior remains valid. | existing correctness fixture | pass |
| `PER-004` | Given a persona-derived population, when a campaign resolves, then the persona and derivation files and digests enter the source identity. | resolver integration test | pass |
| `PER-005` | Given a semantic evaluator proposal, when its persona, derivation, run, or evidence binding is stale or unknown, then aggregation is blocked and no proposal artifact is accepted. | evaluator negative tests | pass |
| `PER-006` | Given a valid proposal, when the run finalizes, then it is stored once under `refinements/` and terminal verification covers it. | artifact integration test | pass |

## Feature Impact Matrix

| Feature / Flow | Contracts | Touched? | Protected Behavior | Required Check | Status |
|---|---|---:|---|---|---|
| Product persona authoring | persona template/index; `compose-spec` | yes | weak evidence is not promoted | marker and validator checks | pass |
| Population resolution | population schema and TypeScript validator | yes | v1 fixtures continue to resolve | definition tests | pass |
| Source identity | campaign source manifest | yes | every referenced source remains digest-bound | resolver/campaign tests | pass |
| Semantic evaluation | output schema/parser/prompt | yes | mechanical gates remain non-compensating | evaluation tests | pass |
| Campaign artifacts | governed namespaces/finalization | yes | lease, exclusivity, redaction, terminal verification | artifact/campaign tests | pass |
| W-004 evidence | WG-001 source-bound receipts | consumer only | blocked gate state is not upgraded | graph/status reconciliation | pass; historical N03 evidence reopened |
...
|---|---|---|---|---|---|
| `W-025-N01` | author versioned product/persona simulation contracts | user authorization | persona docs, schemas, lane/report | `W-025-G01` | accepted |
| `W-025-N02` | implement resolver, dry-run derivation, and proposal artifact wiring | `W-025-G01` | `scripts/cascade/`, evaluator schema | `W-025-G02` | accepted |
| `W-025-N03` | update authoring/execution/evaluation workflow contracts | `W-025-G01` | affected skills/templates/checklists | `W-025-G03` | accepted |
| `W-025-N04` | execute focused and full validation plus fixed-point review | `W-025-G02`, `W-025-G03` | tests, report, status only | `W-025-GT` | accepted |
...
## Terminal Validation And Closeout

- `W-025-G01`, `W-025-G02`, `W-025-G03`, and `W-025-GT`: `ACCEPTED`.
- Deterministic derivation preview: `PASS`; exact approved P-999 manifest,
  persona revision, file digests, allocation semantics, and existing output
  match were verified without writing.
- Focused implementation tests: `53/53 PASS`; artifact hardening subset after
  fixed-point repair: `40/40 PASS`.
- Aggregate repository tests: `80/80 PASS` with Bun `1.3.3`.
- Cascade validator, 44-skill/368-scenario harness catalog, harness self-test,
  ...
  `git diff --check`: `PASS`.
- Fixed-point architecture/security review repaired a generic artifact-store
  bypass by binding proposal JSON to the reserved run, campaign, evaluator,
  and canonical proposal path. Direct mutation remains rejected.
- W-004 remains `BLOCKED`. W-025 changed shared campaign sources, so the
  historical accepted WG-001-N03 receipt is reopened as stale; W-025 does not
  authorize a W-004 retry or accept any WG-001 node.
- Live/provider semantic refinement execution, product-persona realism,
  external-evidence confirmation, target calibration, deployment, and release
  eligibility remain `NOT_RUN`.
- Completion report:
  `docs/work/reports/2026-08-03-persona-simulation-refinement-loop.md`.
- Publication: commit history exists locally; push and external publication
  were not requested.
